...
export const CredentialStore = {
  ...
  mutations: {
    ...
    Mutation___updatePermission: (state, payload) => {
      ...
      const grantedAccess = payload.access
      ...
      for (let a in grantedPerm) {
        ...
        // if(grantedPerm[a].access) {
        //   if(!routeMap[grantedPerm[a].access.identifier]) {
        //     routeMap[grantedPerm[a].access.identifier] = {}
        //   }
        //
        //   if(!routeMap[grantedPerm[a].access.identifier].permission) {
        //     routeMap[grantedPerm[a].access.identifier].permission = []
        //   }
        //
        //   if(routeMap[grantedPerm[a].access.identifier].permission.indexOf(grantedPerm[a].dispatchName) < 0) {
        //     routeMap[grantedPerm[a].access.identifier].permission.push(grantedPerm[a].dispatchName)
        //   }
        // }
      ...
      for(let b in grantedAccess) {
        if(!routeMap[grantedPerm[b].identifier]) {
          routeMap[grantedPerm[b].identifier] = {}
        }

        if(!routeMap[grantedPerm[b].identifier].permission) {
          routeMap[grantedPerm[b].identifier].permission = []
        }

        if(routeMap[grantedPerm[b].identifier].permission.indexOf(grantedPerm[b].dispatchName) < 0) {
          routeMap[grantedPerm[b].identifier].permission.push(grantedPerm[b].dispatchName)
        }
      }
