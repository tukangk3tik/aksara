...
export default {
  ...
  methods: {
    ...
    async getDetail(id) {
      await MasterSekolahService.getSekolahDetail(id).then((detail) => {
        const data = detail.data.payload
        this.formData = {
          npsn: data.npsn,
          nama: data.name,
          bp: data.bp,
          jlh_rombel: data.rombel,
          jlh_guru: data.guru,
    ...
    updateAccountData: function(event) {
      ...
      if (this.allowSave) {
        confirmation.require({
          ...
          accept: async () => {
            await MasterSekolahService.editSekolah(this.$route.query.id, this.formData)
              .then(async (response) => {
                this.$router.push('/master/sekolah')
              })
          },
